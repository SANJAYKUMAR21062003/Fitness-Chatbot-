...
from auth import register_user, authenticate_user, reset_password  # Updated imports
from database import init_db, save_workout_preferences, get_user_id  # Updated imports
import json
import sqlite3
...
def login():
    if request.method == "POST":
        ...
        if authenticate_user(username, password):  # Use the updated authenticate_user from auth.py
            session["user_id"] = get_user_id(username)  # Store user ID in session
...
def signup():
    if request.method == "POST":
        ...
        if register_user(username, email, password):  # Use the updated register_user from auth.py
...
def forgot_password():
    if request.method == "POST":
        email = request.form.get("email")
        if reset_password(email):  # Use the updated reset_password from auth.py
